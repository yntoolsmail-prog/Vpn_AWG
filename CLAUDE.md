...
- **pip — только через `pip_install_sys` (lib/utils.sh):** системный Python Ubuntu 23.04+ требует `--break-system-packages` (старый pip 22.04 флага не знает — ставим без него), а зависимость новее apt-пакета pip обновить не может: «Cannot uninstall X, RECORD file not found» (сентябрь 2026: `anyio` 4.15 из python-telegram-bot требует `typing_extensions` ≥ 4.16, в Ubuntu 24.04 — 4.10; раньше так же `blinker` у flask). Функция ловит это, ставит X свежей копией в `/usr/local` (`--ignore-installed`, apt-пакет не трогается) и повторяет. `setup.sh` перед шагом pip заново подключает `/root/lib/utils.sh`, только что скачанный шагом 10: `lib/` со старта могла остаться от прошлого запуска
- **SSH-меню (`lib/ssh_setup.sh`) работает внутри `setup.sh` под `set -e`:** пункты меню вызываются с `|| true`, иначе неудача (например, apt занят) обрывает весь установщик. apt — с `-o DPkg::Lock::Timeout=600`: на свежем VPS dpkg держит unattended-upgrades, ждём, а не убиваем. Настройки sshd читаются через `_sshd_opt` (`sshd -T`, запасной путь — drop-in `sshd_config.d/*.conf` раньше основного файла), как `get_ssh_password_auth_local()` в Python: строки `Port`/`PermitRootLogin` в `sshd_config` по умолчанию закомментированы, и `grep … | awk … || echo 22` давал пустоту — код конвейера берётся от awk
