...
| Функция | Назначение |
|---------|-----------|
| `ssh_clone_awg_to_slave(server)` | Клонировать AWG-конфиг на slave; сохраняет PostUp/PostDown слейва (у него свой внешний интерфейс; если их нет — строит под его default-интерфейс) и ставит маркер `/etc/awg-slave`. Проверяет результат: код записи конфига, код `awg-quick up`, ключ сервера и Jc/Jmin/Jmax/S1/S2/H1–H4 на интерфейсе слейва против primary (`_OBFS_PARAMS`); при расхождении бросает исключение — слейв с чужими H1–H4 молча отбрасывает пакеты клиентов |
