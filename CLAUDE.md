...
- `tma/index.html` — фронтенд TMA:
  ...
  - `window._monitorPeers` — хранит peers из `d.peers` на экране монитора; позволяет открыть карточку устройства кликом прямо из списка на главном экране
  - Карточка устройства (`openDeviceDetail`) показывает `endpoint` (с какого IP:port заходил) и `server_label` (через какой сервер)
