...
> ⚠️ **Ubuntu 22.04**: перед установкой обязательно обновите систему и перезагрузитесь.
> AmneziaWG компилируется как модуль ядра — на устаревшем ядре компиляция завершится ошибкой.
> ```bash
> apt-get update && apt-get upgrade -y && reboot
> ```
> На **Ubuntu 24.04** этот шаг не требуется — ядро актуально из коробки.
