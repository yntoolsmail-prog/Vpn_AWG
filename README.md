...
## Домены и резервный endpoint

По умолчанию клиентские конфиги используют IP-адрес сервера как `Endpoint`. Вместо этого можно указать домен — тогда при смене IP сервера достаточно обновить DNS-запись, а конфиги на устройствах останутся рабочими.

**Поддерживаются:**
- Собственный домен (Бегет, Timeweb, reg.ru и др.) — создайте A-запись → IP вашего VPS
- DDNS-сервис (DuckDNS, No-IP, DynDNS и др.) — бесплатно, обновляется автоматически

Установщик спросит домен во время установки и проверит что он резолвится в IP сервера.

**Настроить после установки:**
```bash
# Добавить или изменить эндпоинты
...
```
> Новые значения применяются к новым конфигам. Существующим устройствам — скачать `.conf` заново в карточке устройства.

**Резервный endpoint** — если задан, каждый клиент при создании получит два `.conf` файла. В карточке устройства появятся кнопки «Резервный .conf» и «Резервный QR».
