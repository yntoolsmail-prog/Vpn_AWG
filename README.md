...
Скрипт `/root/update.sh` запускается каждые 5 минут через cron, проверяет последний коммит в `main` и при изменении скачивает новые версии `bot.py` и `vpn.sh`, затем перезапускает бота.

```bash
crontab -l | grep update.sh          # проверить задачу в cron
...
> Автообновление затрагивает только `bot.py` и `vpn.sh`. Конфиги, ключи и данные клиентов не изменяются.
