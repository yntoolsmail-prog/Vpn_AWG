...
async def show_help(query):
    ...
    text = (
        "📖 *Инструкция*\n\n"
        "▶️ *ОТКРЫТЬ VPN* 🔑 — открыть веб-панель управления прямо в Telegram.\n\n"
        "🧲 *Добавить устройство* — создать VPN-профиль для телефона, ноутбука, ПК и т.д.\n\n"
        "📋 *Мои устройства* — ваши профили. Нажмите на устройство чтобы:\n"
        "• 📄 скачать *.conf* файл — для AmneziaWG\n"
        "• 📱 получить *QR-код* — для AmneziaWG на телефоне\n"
        "• 📤 *Поделиться кодом* — для AmneziaVPN\n"
        "• 🌐 *Исключения сайтов* — банки, госуслуги и другие сайты без VPN\n"
        "• 🗑 удалить устройство\n\n"
        "📊 *Статус сервера* — онлайн-устройства и нагрузка сервера.\n\n"
        "⚠️ *Важно — на каждый гаджет свой профиль!*\n"
        "Один конфиг на двух устройствах — оба будут глючить.\n\n"
        "📲 *Как подключиться:*\n"
        "1. 🧲 Добавить устройство → введите название (`Phone`, `iPad`, `PC`)\n"
        "2. 📋 Мои устройства → откройте карточку\n"
        "3. Скачайте *.conf* или QR-код\n"
        "4. Установите *AmneziaWG* → импортируйте файл → включите тоннель\n\n"
        "📱 *Приложения:*\n"
        "• *AmneziaWG* — рекомендуется (Android, iOS, Windows, Mac)\n"
