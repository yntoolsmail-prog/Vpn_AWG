...
def _write_mtp_service(port: str, secret: str):
    ...
    with open(f"/etc/systemd/system/{MTP_SERVICE}.service", "w") as f:
        f.write(
            ...
            f"[Service]\n"
            f"ExecStartPre=-/bin/sh -c 'curl -sf --max-time 10 https://core.telegram.org/getProxySecret"
            f" -o {MTP_SECRET_F}; curl -sf --max-time 10 https://core.telegram.org/getProxyConfig"
            f" -o {MTP_MULTI_F}'\n"
