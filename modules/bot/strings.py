...
# ── Справка (/help) ────────────────────────────────────────────────────────────
def get_help_main(tma_url: str = "", awg3: bool = False) -> str:
    lines = ["📖 *Инструкция*\n"]
    if tma_url:
        lines.append("▶️ *Веб интерфейс* 🔑 — веб-панель управления прямо в Telegram.")
    lines += [
    ...
    if awg3:
        # Сервер на протоколе AmneziaWG 3.1: старые версии приложений конфиг не примут
        lines += [
            "🆕 *Сервер работает на протоколе AmneziaWG 3.1.*",
            "Нужны свежие версии: *AmneziaVPN 5.0.1.5+* или *AmneziaWG 3.1+*."
            " Старые приложения, роутеры и сторонние клиенты без поддержки 3.1"
            " не подключатся — обновите приложение перед импортом.\n",
        ]
    lines += [
        "━━━━━━━━━━━━",
        "📲 *Подключение через AmneziaVPN:*",
        "1. Установите *AmneziaVPN* (Android, iOS, Windows, Mac, Linux — amnezia.org)",
